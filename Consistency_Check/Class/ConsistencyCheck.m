    ...
    methods 
        ...
        function [eqSys, eqVal, maxVehicle] = performConsistencyCheck(obj, currentStateTable, oldStateTable, interval)
            ...
            for vIndex = 1 : maxVehicle
                ...
                yawRateOld = deg2rad(oldStateTable(vIndex, SimulationVehicleData.TABLE_INDEX_YAW_RATE));
                headingOld = deg2rad(oldStateTable(vIndex, SimulationVehicleData.TABLE_INDEX_H));
...
%             end
        ...
        function isAnomalous = checkYawRateHeading(obj, headingOld, headingNew, yawRateOld, interval)
            ...
            if abs(headingNew - headingOld) < pi
                yawRateEst = wrapToPi(headingNew - headingOld) / interval;            
            else
                yawRateEst = wrapToPi(wrapTo2Pi(headingNew) - wrapTo2Pi(headingOld)) / interval;
            end
        ...
        function isAnomalous = checkLocHeading(obj, locOld, locNew, headingOld)
            ...
            if abs(headingEst - headingOld) > pi
                headingEst = wrapTo2Pi(headingEst);
                headingOld = wrapTo2Pi(headingOld);
            end
